...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o nonprofitvault-backend .
...
COPY --from=build-env /app/nonprofitvault-backend .
...
CMD [ "/nonprofitvault-backend" ]

# BUILD
# (Run following from project root directory!)
# docker build -f Dockerfile -t LuchaComics/nonprofitvault-backend:latest --platform linux/amd64 .

# EXECUTE
# docker tag LuchaComics/nonprofitvault-backend:latest LuchaComics/nonprofitvault-backend:latest

# UPLOAD
# docker push LuchaComics/nonprofitvault-backend:latest
