...
import (
	...
	sub_c "github.com/Pharo-Non-Profit/nonprofitvault-backend/internal/app/objectfile/controller"
...
func UnmarshalUpdateRequest(ctx context.Context, r *http.Request) (*sub_c.ObjectFileUpdateRequestIDO, error) {
	defer r.Body.Close()
	...
	requestData := &a_c.ObjectFileUpdateRequestIDO{
		ID:            aid,
		Name:          name,
...
func (h *Handler) UpdateByID(w http.ResponseWriter, r *http.Request, id string) {
	...
	objectfile, err := h.Controller.UpdateByID(ctx, data)
	if err != nil {
		httperror.ResponseError(w, err)
		return
	}

	MarshalUpdateResponse(objectfile, w)
}

func MarshalUpdateResponse(res *sub_s.ObjectFile, w http.ResponseWriter) {
