...
// SmartFolderPaginationListResult represents the paginated list results for
...
func (impl SmartFolderStorerImpl) newPaginationFilter(f *SmartFolderPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl SmartFolderStorerImpl) newPaginationFilterBasedOnString(f *SmartFolderPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl SmartFolderStorerImpl) newPaginationFilterBasedOnInt8(f *SmartFolderPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl SmartFolderStorerImpl) newPaginationFilterBasedOnTimestamp(f *SmartFolderPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl SmartFolderStorerImpl) newPaginationOptions(f *SmartFolderPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl SmartFolderStorerImpl) newPaginatorNextCursor(f *SmartFolderPaginationListFilter, results []*SmartFolder) (string, error) {
	var lastDatum *SmartFolder
