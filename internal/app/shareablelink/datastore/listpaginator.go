...
func (impl ShareableLinkStorerImpl) newPaginationFilter(f *ShareableLinkPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
		...
		default:
			return nil, fmt.Errorf("unsupported sort field for `%v`, only supported field is `created_at`", f.SortField)
		}
	}
	return bson.M{}, nil
}
...
func (impl ShareableLinkStorerImpl) newPaginatorNextCursor(f *ShareableLinkPaginationListFilter, results []*ShareableLink) (string, error) {
	...
	case "created_at":
		...
		break
	default:
		return "", fmt.Errorf("unsupported sort field in options for `%v`, only supported field is `created_at`", f.SortField)
	}
